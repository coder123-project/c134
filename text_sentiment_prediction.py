...
def predict(text):
    ...
    if  text!="":
        ...
        # sequences = tokenizer.sequences(sentence)
        # sequences = tokenizer.(sentence)
        # sequences = tokenizer.texts_sequences(sentence)
       ...
       # predicted_class_label = argmax(model.predict(testing_padded), axis=1)       
       #     # predicted_class_label = np.arraymax(model.predict(testing_padded), axis=1)  
       #     # predicted_class_label = np.argmax(model.predict(testing_padded), axis=1)  
       #     # predicted_class_label = np.max(model.predict(testing_padded), axis=1)   
